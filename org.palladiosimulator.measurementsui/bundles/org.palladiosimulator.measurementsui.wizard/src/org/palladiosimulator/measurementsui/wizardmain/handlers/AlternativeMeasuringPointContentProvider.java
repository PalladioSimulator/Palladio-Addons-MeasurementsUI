...
elementList.addAll(da.getModelAccessor().getAllocation().stream().filter(e->e instanceof Repository).collect(Collectors.toCollection(LinkedList::new)));
    ...
    public Object[] getChildren(Object parentElement) {
        if(parentElement instanceof Allocation) {
           ...
           return elements.toArray();
            ...
            return ((ScenarioBehaviour)parentElement).getActions_ScenarioBehaviour().stream().filter(e->e instanceof EntryLevelSystemCall).collect(Collectors.toCollection(LinkedList::new)).toArray();
        }else if (parentElement instanceof Repository) {

            return ((Repository)parentElement).eContents().stream().filter(e -> e instanceof BasicComponent).collect(Collectors.toCollection(LinkedList::new)).toArray();
            ...
            return ((ResourceDemandingSEFF)parentElement).getSteps_Behaviour().stream().filter(e -> e instanceof ExternalCallAction).collect(Collectors.toCollection(LinkedList::new)).toArray();
        }
